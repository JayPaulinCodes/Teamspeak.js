...
function makeErrorClass(BaseClass) {
    return class TeamspeakJsError extends BaseClass {
        ...
        static message(code: string, args) {
            if (!(code in TeamspeakJsErrorCodes)) { throw new Error("Error code must be a valid TeamspeakJsErrorCodes"); }
        
            const errorMessage: string | Function | undefined = TeamspeakJsErrorMessages[code];
            if (!errorMessage) { throw new Error(`No message associated with error code: ${code}.`); }
                
            if (typeof errorMessage === "function") { return errorMessage(...args); }
            if (!args?.length) { return errorMessage; }
